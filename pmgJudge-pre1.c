    ...
    for(int l = 0; l < AllLay; l++) {
        for(int i = 0; i < size; i++) {
            for(int j = 0; j < size; j++) {
                synapse[l][i][j]=randn(0.0f, 0.05f); //He初期化
            }
        ...
        for(int l=0; l < AllLay; l++) {
            for(int i=0;i<size;i++){ 
                for(int j =0;j<size;j++){
                    ...
                    float learn_scale=0.001f;
                    
                    synapse[l][i][j] -= totalSy *learn_scale / (float)time;
                    bias[l][j] -= totalBi *learn_scale/ (float)time; //バイアスの更新
                }
            }
