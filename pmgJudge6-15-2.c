...
int first(int importflag){
    ...
    if(strcmp(mode, "reset") == 0){
        ...
        printf("エポックの損失関数%lf\n", totalCost/(count/costNo));
        printf("エポックの正答率%f%%\n", corectRate/(count/costNo));
...
void judge(const char *filename) {
    ...
    // 損失関数
        ...
        double *loss = (double*)malloc(sizeof(double) * costNo);
        ...
        if(time == costNo-1){
            ...
            totalCost += sum/(double)costNo;
            corectRate += (float)correct/(float)costNo*100;
            
            printf("損失関数%lf\n", sum/(double)costNo);
            printf("正答率%f%%\n",(float)correct/(float)costNo*100);
            correct=0;
            printf("シナプスとバイアスを更新しています...");
            //write_log(filename, max, judgelay, 100);
        }

        backprop(answer); // 誤差逆伝播法を実行
    ...
    if(time >= costNo) {
        ...
        for(int l=0; l < AllLay; l++) {
            ...
            for(int i=0;i<node_num;i++){ 
                ...
                bias[l][i] -= totalBias[l][i] * learn_scale / (double)costNo; // バイアスの更新
                totalBias[l][i] = 0.0f;
                ...
                for(int j =0;j<size;j++){
                    ...
                    synapse[l][j][i] -= totalSynapse[l][j][i] * learn_scale / (double)costNo; //シナプスの更新
