...
#define CLIP_MAX 1.0
#define CLIP_MIN -1.0
#define CLIP_ZERO 1e-16
#define LAMBDA 0.01
#define DROPOUT_RATE 0.5
int* read_pgm(const char* filename);
int view_pgm(const char *filename);
void reset();
void judge(const char *filename,int teacher,double learn_scale);
int first(int importflag);
double* layOpt(int layNo,double *input,int teacher);
...
void allmalloc(int teacher);
void backprop(int answer,double learn_scale);
int save(const char *filename);
int import(const char *filename);
int learning(const char *folderpath,double learn_scale);
...
int first(int importflag){
    ...
    if(strcmp(mode, "reset") == 0){
        ...
        count=learning(folderpath,learn_scale);

        printf("エポックの損失関数%lf\n", totalCost/(count/BatchSize));
        printf("エポックの正答率%f%%\n", corectRate/(count/BatchSize));
...
void judge(const char *filename,int teacher,double learn_scale) {
    ...
    for(int l = 0; l < AllLay; l++) {
        ...
        if(l == AllLay-1) {
            ...
            double *tmp = layOpt(l,lay[l],teacher);
            for(int i = 0; i < 10; i++) {
                judgelay[i] = tmp[i];
            }
    ...
    // 損失関数
        ...
        double *loss = (double*)malloc(sizeof(double) * BatchSize);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (judgelay[i] > 0.0f) {
                loss[time] -= y * log(judgelay[i])+(1.0f - y) * log(1.0-judgelay[i]);
            }
        ...
        if(time == BatchSize-1){
            ...
            totalCost += sum/(double)BatchSize;
            corectRate += (float)correct/(float)BatchSize*100;
            
            printf("損失関数%lf\n", sum/(double)BatchSize);
            printf("正答率%f%%\n",(float)correct/(float)BatchSize*100);
            correct=0;
            printf("シナプスとバイアスを更新しています...");
            //write_log(filename, max, judgelay, 100);
        }

        backprop(answer,learn_scale); // 誤差逆伝播法を実行
...
double* layOpt(int l,double *input,int teacher){ 
    ...
    if(l!=AllLay-1){
        for(int i = 0; i < size; i++) {
            ...
            double sum = 0.0f;
                ...
                for(int j = 0; j < size; j++) {
                    sum += input[j]*synapse[l][j][i];
                }
                sum = sum + bias[l][i];
                
                total[i] = fmaxf(0.0f, sum);//ReLUに通す

                //if(teacher) {
                 //   total[i] *=(double)dropout_mask[l][i];
        ...
        for(int i = 0; i < 10; i++) {
            ...
            double sum =0.0f;
                ...
                total[i] = sum + bias[l][i];

                if(total[i]>CLIP_MAX) total[i]=CLIP_MAX;

                if(total[i]<CLIP_MIN) total[i]=CLIP_MIN;


                if(total[i]==0) total[i]=CLIP_ZERO;
    ...
    if(time >= BatchSize) {
        ...
        for(int l=0; l < AllLay; l++) {
            ...
            for(int i=0;i<node_num;i++){ 
                ...
                bias[l][i] -= totalBias[l][i] * learn_scale / (double)BatchSize; // バイアスの更新
                totalBias[l][i] = 0.0f;
                ...
                for(int j =0;j<size;j++){
                    ...
                    synapse[l][j][i] -= totalSynapse[l][j][i] * ((learn_scale / (double)BatchSize) ); //シナプスの更新
        ...
        time = 0; // 時間がBatchSizeを超えたらリセット
        printf("ニューロン及びバイアスを更新しました。\n");
    }
