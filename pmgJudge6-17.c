...
int main() {
   ...
   int temp = 1;
    ...
    while(temp==1){
        temp = first(importflag,saveflag);
        importflag=1;
    ...
    if(l!=AllLay-1){
        ...
        for(int i = 0; i < 10; i++) {
            ...
            total[i] = 0.0f;
                ...
                if(total[i]<0.0&&total[i]>-1.0e-7)total[i]=-1e-7f;
                if(total[i]>0.0&&total[i]<1.0e-7)total[i]=1e-7f;
        ...
        if(sum<0.0&&sum>-1.0e-8)sum=-1.0e-8;
        if(sum>0.0&&sum<1.0e-8)sum=1.0e-8;
...
void backprop(int t,int answer) {
    ...
    #pragma omp parallel for default(none) shared(delta,judgelay, answer, totalBias, totalSynapse, lay, total, size, l)
            for (int i = 0; i < 10; i++) { //layの値を0に初期化 //iは出力元
                ...
                sum =  (judgelay[i]-y);// 誤差を計算
                totalBias[l][i] += sum; // バイアスの更新
    ...
    if(time >= batchNum) {
        ...
        for(int l=0; l < AllLay; l++) {
            ...
            for(int i=0;i<node_num;i++){ 
                ...
                bias[l][i] -= totalBias[l][i] * (double)learn_scale / (double)batchNum; // バイアスの更新
                totalBias[l][i] = 0.0f;


                for(int j =0;j<size;j++){
                    double threshold = 1.0f; // シナプスの閾値
                    if (totalSynapse[l][j][i] > threshold) totalSynapse[l][j][i] = threshold;
                    if (totalSynapse[l][j][i] < -threshold) totalSynapse[l][j][i] = -threshold;
        
                    synapse[l][j][i] -= (totalSynapse[l][j][i] * (double)learn_scale / (double)batchNum); //シナプスの更新
...
int learning(const char *folderpath) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        free(filelist);
    printf("現在設定 batchsize%d 学習率 %f\n", batchNum, learn_scale);
